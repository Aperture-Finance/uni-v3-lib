...
contract PoolCallerTest is BaseTest {
    ...
    function prepSwap(
        ...
        if (zeroForOne) {
            amountSpecified = bound(
                amountSpecified,
                1,
                IERC20(token0).balanceOf(pool) / 10
            );
            ...
            amountSpecified = bound(
                amountSpecified,
                1,
                IERC20(token1).balanceOf(pool) / 10
            );
    ...
    function test_Slot0() public {
        ...
        assertEq(
            ...
            observationCardinalityAsm,
            "observationCardinality"
        );
        assertEq(
            observationCardinalityNext,
            observationCardinalityNextAsm,
            "observationCardinalityNext"
        );
    ...
    function test_SqrtPriceX96AndTick() public {
        (uint160 sqrtPriceX96, int24 tick, , , , , ) = IUniswapV3Pool(pool)
            .slot0();
        (uint160 sqrtPriceX96Asm, int24 tickAsm) = V3PoolCallee
            .wrap(pool)
            .sqrtPriceX96AndTick();
        assertEq(sqrtPriceX96, sqrtPriceX96Asm, "sqrtPriceX96");
        assertEq(tick, tickAsm, "tick");
    ...
    function testFuzz_TickBitmap(int16 wordPos) public {
        assertEq(
            IUniswapV3Pool(pool).tickBitmap(wordPos),
            poolCallee.tickBitmap(wordPos),
            "tickBitmap"
        );
    }
