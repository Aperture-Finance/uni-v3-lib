...
contract BitMathTest is IBitMath {
    function mostSignificantBit(uint256 x) external pure override returns (uint8 r) {
        return BitMath.mostSignificantBit(x);
    }

    function leastSignificantBit(uint256 x) external pure override returns (uint8 r) {
